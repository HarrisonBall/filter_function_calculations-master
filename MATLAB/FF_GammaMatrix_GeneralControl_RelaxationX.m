...
function [FF_GAMMA_MATRIX_GENERAL_CONTROL_RELAXATION_X] = FF_GammaMatrix_GeneralControl_RelaxationX(GammaMatrix,w_col)
...
% FF_GammaMatrix_RelaxationX(GammaMatrix,w_col)
...
%noise in sigma_x (relaxation) quadrature. The control Hamiltonian is 
%assumed to satisfy the GENERAL control form. 
...
%              NUMERICALLY COMPUTE RELAXATION X FILTER FUNCTION  
%__________________________________________________________________________
...
Rx = zeros(length(w_col),3);

%--------------------------------------------------------------------------
%               COMPUTE TOTAL RELAXATION X CONTROL VECTOR  
%                       see Eq. 127 of Ref. [1] 
%--------------------------------------------------------------------------
...
for l=1:N;                            
    ...
    %GammaMatrix_Row = GammaMatrix(l,:);
    
    RxPl = Local_RelaxationXControlVector_GeneralControl_GammaMatrix(GammaMatrix,l,w_col);
    
    Lambda_lm1 = HistoryMatrix_GammaMatrix(GammaMatrix,l-1);
        
    Update = EXPtlm1*RxPl*Lambda_lm1;
...
FF_GAMMA_MATRIX_GENERAL_CONTROL_RELAXATION_X = Filt_x;
