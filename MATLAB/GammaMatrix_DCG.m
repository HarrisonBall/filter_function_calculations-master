function [GAMMA_DCG] = GammaMatrix_DCG(pitime)
...
%This m-file outputs the standard GammaMatrix-parametrization of a DCG NOT
%gate. The DCG NOT gate enacts a net pi-rotation about the sigma_x axis of 
%the Bloch sphere using a composite sequence of 3 distinct pi-rotations. Let 
%Rx(theta_k,tau_k) be a rotation of the Bloch vector about the sigma_x axis 
%through an angle theta_k, over a duration tau_k during the time interval
%[t_{k-1},t_{k}]. Then the the DCG NOT gate is implemented by the following
%sequence of rotations:

%               Rx(pi, tau/4)Rx(pi, tau/2)Rx(pi, tau/4)
...
Omega_col = [pi/pitime, pi/(2*pitime), pi/pitime]';

tau_col = pi*ones(3,1)./Omega_col;

theta_col = Omega_col.*tau_col;
 
nhat_col = [1,0,0;1,0,0;1,0,0];
 
GAMMA_DCG  = cat(2,Omega_col,tau_col,theta_col,nhat_col);
