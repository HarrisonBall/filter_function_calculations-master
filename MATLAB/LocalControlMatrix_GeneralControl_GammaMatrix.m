...
function  LOCAL_CONTROL_MATRIX_GENERAL_CONTROL_GAMMA_MATRIX = LocalControlMatrix_GeneralControl_GammaMatrix(GammaMatrix,l,w_col)
...
% LocalControlMatrix_GeneralControl_GammaMatrix(GammaMatrix,1,w_col)
% 
...
%This function outputs the w-domain local control matrix (see Eq. 128 in 
%Ref. [1]) associated with the "lth" pulse segment of a piecewise-constant 
...
%                  LOCAL CONTROL MATRIX ELEMENT FORMULAE
%__________________________________________________________________________
...
RyxPlwGENERAL= FRAC.*(G(l,1).*(1i.*w_col.*G(l,6)-G(l,4).*G(l,5).*G(l,1))+exp(1i.*w_col.*G(l,2)).*(-w_col.*G(l,6).*(w_col.*sin(G(l,3))+1i.*cos(G(l,3)).*G(l,1))+G(l,4).*G(l,5).*(w_col.^2.*(-1+cos(G(l,3)))-1i.*w_col.*sin(G(l,3)).*G(l,1)+G(l,1).^2)));

RyyPlwGENERAL= FRAC.*(w_col.^2-G(l,5).^2.*G(l,1).^2+exp(1i.*w_col.*G(l,2)).*(w_col.^2.*cos(G(l,3)).*(-1+G(l,5).^2)-1i.*w_col.*sin(G(l,3)).*(-1+G(l,5).^2).*G(l,1)+G(l,5).^2.*(-w_col.^2+G(l,1).^2)));

RyzPlwGENERAL= FRAC.*(-G(l,1).*(1i.*w_col.*G(l,4)+G(l,5).*G(l,6).*G(l,1))+exp(1i.*w_col.*G(l,2)).*(w_col.*G(l,4).*(w_col.*sin(G(l,3))+1i.*cos(G(l,3)).*G(l,1))+G(l,5).*G(l,6).*(w_col.^2.*(-1+cos(G(l,3)))-1i.*w_col.*sin(G(l,3)).*G(l,1)+G(l,1).^2)));

RzxPlwGENERAL= FRAC.*(-G(l,1).*(1i.*w_col.*G(l,5)+G(l,4).*G(l,6).*G(l,1))+exp(1i.*w_col.*G(l,2)).*(w_col.*G(l,5).*(w_col.*sin(G(l,3))+1i.*cos(G(l,3)).*G(l,1))+G(l,4).*G(l,6).*(w_col.^2.*(-1+cos(G(l,3)))-1i.*w_col.*sin(G(l,3)).*G(l,1)+G(l,1).^2)));

RzyPlwGENERAL= FRAC.*(G(l,1).*(1i.*w_col.*G(l,4)-G(l,5).*G(l,6).*G(l,1))+exp(1i.*w_col.*G(l,2)).*(-w_col.*G(l,4).*(w_col.*sin(G(l,3))+1i.*cos(G(l,3)).*G(l,1))+G(l,5).*G(l,6).*(w_col.^2.*(-1+cos(G(l,3)))-1i.*w_col.*sin(G(l,3)).*G(l,1)+G(l,1).^2)));

RzzPlwGENERAL= FRAC.*(w_col.^2-G(l,6).^2.*G(l,1).^2+exp(1i.*w_col.*G(l,2)).*(w_col.^2.*cos(G(l,3)).*(-1+G(l,6).^2)-1i.*w_col.*sin(G(l,3)).*(-1+G(l,6).^2).*G(l,1)+G(l,6).^2.*(-w_col.^2+G(l,1).^2)));
...
LOCAL_CONTROL_MATRIX_GENERAL_CONTROL_GAMMA_MATRIX = [RxxPlwGENERAL RxyPlwGENERAL RxzPlwGENERAL; RyxPlwGENERAL RyyPlwGENERAL RyzPlwGENERAL; RzxPlwGENERAL RzyPlwGENERAL RzzPlwGENERAL];
